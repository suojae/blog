...
## SvgPicture는 아무것도 안 한다

`SvgPicture`는 `StatelessWidget`이다. 상태도 없고 렌더링 로직도 없다. `build()` 메서드 하나가 전부인데, 거기서 하는 일은 모든 파라미터를 다른 패키지의 위젯에 그대로 넘기는 것이다.
...
그럼 왜 존재하나? **"어디서 SVG를 가져올지"를 결정하는 인터페이스 계층**이기 때문이다. 사용자에게 `.asset()`, `.network()`, `.file()` 같은 편리한 입구를 제공하고, 실제 렌더링은 범용 벡터 그래픽 엔진에 맡긴다.
...
`SvgPicture`에는 생성자가 6개 있다. 기본 생성자 1개와 named 생성자 5개. 차이점은 **내부에서 어떤 Loader를 만드느냐** 뿐이고, 그 이후의 파이프라인은 전부 동일하다.

| 생성자 | 내부 Loader | 데이터 소스 |
|---|---|---|
...
Loader들은 모두 `SvgLoader<T>`를 상속한다. 이 추상 클래스에는 2단계 파이프라인이 있다:
...
**왜 2단계로 나눴나?** `prepareMessage()`는 `BuildContext`에 접근해야 하니까 메인 스레드에서 실행된다. 무거운 XML 파싱과 바이너리 컴파일은 `compute()`로 별도 Isolate에 보내서 UI를 막지 않는다.
...
**왜 바이너리로 변환하나?** XML은 파싱이 느리다. 바이너리는 별도 파싱 없이 바로 읽을 수 있다. 한 번 컴파일해서 캐시하면 두 번째부터는 훨씬 빠르다.

---

## 바이너리가 그리기 명령어로 풀리는 과정

Loader가 바이너리를 반환하면, 렌더링 엔진이 이걸 Flutter Canvas 명령어로 변환한다. **Listener 패턴**을 쓴다.
...
모든 그리기 명령이 `PictureRecorder`에 녹화된다. 녹화가 끝나면 `Picture` 객체가 완성된다.

**Picture란?** Canvas에 그린 모든 명령어를 담고 있는 객체다. 아직 픽셀은 아니다. "빨간 선을 여기서 저기로 그어라" 같은 **레시피**다. 나중에 화면에 "재생"하면 그때 픽셀이 된다.

---

## 두 겹의 캐시가 같은 SVG를 다시 불러올 때 속도를 바꾼다

같은 SVG를 화면 여러 곳에서 쓸 때, 매번 처음부터 로딩하면 느리다. 그래서 2단계 캐시가 있다.
...
**왜 캐시가 두 겹인가?** 관리 주체가 다르다. 1단계는 SVG 파싱 패키지가, 2단계는 렌더링 엔진이 각자 관리한다. 그리고 캐시 키가 다르다. 같은 SVG라도 테마가 바뀌면 1단계 캐시가 미스나고, locale이 바뀌면 2단계 캐시가 미스난다.

### LRU 캐시 (1단계)

최대 100개를 저장한다. 가득 차면 가장 오래 안 쓴 항목을 버린다. 기존 항목이 다시 쓰이면 맨 뒤로 이동한다.
...
C를 다시 사용 → [A, B, D, ... Y, Z, C]
새 항목 추가  → [B, D, ... Y, Z, C, 새항목]  (A 삭제)
```

### 참조 카운팅 (2단계)

같은 아이콘이 화면에 3개 있으면, Picture를 3번 디코딩하지 않고 1개를 공유한다. 각 위젯이 마운트될 때 count +1, 디스포즈될 때 count -1. count가 0이 되면 캐시에서 삭제하고 GPU 메모리를 해제한다.
...
Icon A 사라짐 → count: 2 (B, C가 쓰고 있으니 캐시 유지)
Icon B 사라짐 → count: 1
...
두 번째 사용:
  ...
  → 1단계 캐시 히트! (메모리 읽기, 즉시)
  → 2단계 캐시 히트! (메모리 읽기, 즉시)
  → RenderObject → 화면 (GPU만)
```

디스크 I/O, XML 파싱, 컴파일, 디코딩 전부 건너뛴다.

---

## 매번 그리냐, 사진 찍어 재사용하냐

디코딩이 끝나면 `build()`에서 위젯 트리를 구성한다. 여기서 렌더링 전략이 갈린다.

### Picture 전략 (벡터)

매 프레임마다 `canvas.drawPicture()`로 명령어를 재실행한다. 명령어이기 때문에 확대해도 다시 계산해서 깨끗하게 그린다. 하지만 명령어가 수천 개면 매 프레임마다 수천 개를 실행하니까 느려질 수 있다.

### Raster 전략 (비트맵)

최초 1회만 명령어를 실행해서 비트맵 이미지를 찍어둔다. 이후에는 그 이미지를 화면에 붙이기만 한다. 빠르지만 확대하면 픽셀이 보인다.

| | Picture | Raster |
|---|---|---|
| 비유 | 주문 때마다 요리하는 식당 | 미리 만든 도시락 |
| 장점 | 확대해도 선명 | 성능 좋음 |
| 단점 | 복잡한 그래픽은 느림 | 확대 시 깨짐 |
| 적합한 용도 | 아이콘, 로고 (작고 다양한 크기) | 배경, 일러스트 (크고 고정 크기) |

`SvgPicture` 경유 시 기본값은 picture다. 아이콘이나 로고 같은 작은 SVG에 많이 쓰이니까, 다양한 크기로 쓰여도 선명한 게 더 중요하다.
...
사용자가 width/height 중 하나만 지정하면 SVG 원본의 종횡비를 유지하며 나머지를 계산한다. `FittedBox`가 지정된 공간에 맞게 SVG를 스케일하고 정렬한다.

RTL 언어에서 아이콘이 좌우 반전되어야 할 때는 `matchTextDirection: true`로 Transform을 추가한다.
...
두 번째 사용 시에는 캐시 덕분에 로딩 → 컴파일 → 디코딩이 전부 건너뛰어져서 거의 즉시 표시된다.

---

## 테마가 SVG의 색상을 바꿀 수 있다

SVG에서 `currentColor`를 쓰면 외부에서 색상을 주입할 수 있다. `SvgTheme`이 이 역할을 한다:

```dart
SvgTheme(
  currentColor: Colors.blue,  // SVG의 currentColor를 파란색으로
  fontSize: 14,               // em 단위 계산 기준
)
```

테마 적용 우선순위:
1. 생성자에서 직접 지정한 theme
2. `DefaultSvgTheme` InheritedWidget으로 상위에서 내려온 테마
3. 기본값 (검정, 14px)
...
같은 SVG라도 테마가 다르면 캐시 키가 달라진다. 컴파일 결과가 다르기 때문이다.

더 세밀한 색상 제어가 필요하면 `ColorMapper`를 쓴다. `colorFilter`가 렌더링 시점에 전체 일괄 적용인 반면, `ColorMapper`는 파싱 시점에 개별 요소의 색상을 하나하나 변환한다.
